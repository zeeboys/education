...
export class ContractClient {
  ...
  constructor() {
    ...
    this.networkPassphrase = process.env.NEXT_PUBLIC_STELLAR_NETWORK === 'PUBLIC' 
      ? 'PUBLIC'
      : 'TESTNET';

    this.contracts = {
      eduTokenContractId: process.env.NEXT_PUBLIC_EDU_TOKEN_CONTRACT_ID || '',
      bountyEscrowContractId: process.env.NEXT_PUBLIC_BOUNTY_ESCROW_CONTRACT_ID || '',
  ...
  async getEduTokenBalance(address: string): Promise<string> {
    try {
      ...
      // Simulate contract call to get balance
